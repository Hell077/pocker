...
export default function ActionPanel() {
    ...
    const handleRaiseClick = () => setShowModal(true)
    const handleConfirm = () => {
        console.log('Raise for', raiseAmount)
        setShowModal(false)
    }
    ...
    return (
        <>
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 flex gap-4">
                <button className="px-6 py-2 rounded-full bg-red-700 hover:bg-red-600 text-white font-semibold shadow-md">
                    Fold
                </button>
                <button className="px-6 py-2 rounded-full bg-blue-600 hover:bg-blue-500 text-white font-semibold shadow-md">
                    Call
                </button>
                <button
                    ...
                    <div className="bg-[#111] border border-yellow-400 rounded-xl shadow-[0_0_30px_#facc15] p-6 w-[340px] animate-fadeIn text-center">
                        ...
                        <div className="flex items-center justify-between gap-2 bg-black border border-yellow-500 rounded px-2 py-1 mb-4">
                            ...
                            <button
                                ...
                                onClick={() => setRaiseAmount((prev) => Math.max(minRaise, prev - step))}
                            ...
                            <button
                                ...
                                onMouseUp={stopInterval}
                                onMouseLeave={stopInterval}
                                onClick={() => setRaiseAmount((prev) => Math.min(maxRaise, prev + step))}
                                className="text-yellow-400 text-xl font-bold px-2 hover:text-yellow-300"
                            >
                                +
                            </button>
