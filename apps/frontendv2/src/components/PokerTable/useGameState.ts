...
export const useGameState = () => {
  ...
  const [winnerPayload, setWinnerPayload] = useState<WinnerPayload | null>(null);
    ...
    ws.onmessage = async (event) => {
      ...
      if (text.startsWith('{')) {
        try {
          ...
          if (json.type === 'winner') {
            const winnerId = json.winnerId ?? null;
            ...
            try {
              const res = await fetch(`${API_URL}/room/players-by-id`, {
                ...
                body: JSON.stringify({ user_id: [winnerId] }),
              });

              if (!res.ok) {
                console.warn('⚠️ Не удалось получить имя победителя:', await res.text());
                return;
              }

              const users: Player[] = await res.json();
              const winner = users.find(u => u.id === winnerId);
              const username = winner?.nickname ?? winnerId;

              setWinnerPayload({ nickname: username, winnerId, amount, type: 'winner', message: `🎉 Победитель: ${username}!` });
              ...
              console.warn('⚠️ Ошибка запроса победителя:', e);
            }
